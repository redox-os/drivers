...
pub trait Hardware: Sized {
    ...
    fn try_submit(ctxt: &Self::GlobalCtxt, sq_id: Self::SqId, success: impl FnOnce(Self::CmdId) -> Self::Sqe, fail: impl FnOnce()) -> Option<(Self::CqId, Self::CmdId)>;
...
pub struct LocalExecutor<Hw: Hardware> {
    ...
    awaiting_completion: RefCell<HashMap<Hw::CqId, HashMap<Hw::CmdId, (FutIdx, NonNull<Option<Hw::Cqe>>)>>>,
...
impl<Hw: Hardware> LocalExecutor<Hw> {
    pub fn register_external_event(&self, fd: usize, flags: event::EventFlags) -> ExternalEventSource<Hw> {
    ...
    pub fn poll(&self) -> usize {
        ...
        for future_idx in self.ready_futures.borrow_mut().drain(..) {
            ...
            let res = match std::panic::catch_unwind(AssertUnwindSafe(|| futures[future_idx].as_mut().poll(&mut task::Context::from_waker(&waker)))) {
                Ok(r) => r,
                Err(_) => {
    ...
    fn react(&self) {
        ...
        Hw::poll_cqes(&self.global_ctxt, |cq_id, cqe| {
            if let Some((fut_idx, comp_ptr)) = self.awaiting_completion.borrow_mut().get_mut(&cq_id).and_then(|per_cmd| per_cmd.remove(&Hw::get_cqe_cmdid(&cqe))) {
                unsafe {
                    comp_ptr.as_ptr().write(Some(cqe));
                }
                self.ready_futures.borrow_mut().push_back(fut_idx);

                if let Some(submitting) = self.awaiting_submission.borrow_mut().get_mut(&Hw::sq_cq(&self.global_ctxt, cq_id)).and_then(|q| q.pop_front()) {
...
fn current_executor_and_idx<Hw: Hardware>(cx: &mut task::Context<'_>) -> (Rc<LocalExecutor<Hw>>, FutIdx) {
    let executor = LocalExecutor::current();

    let idx = cx.waker().as_raw().data() as FutIdx;
    assert_eq!(cx.waker().as_raw().vtable() as *const _, Hw::vtable(), "incompatible executor for CqeFuture");
    ...
    fn poll(self: Pin<&mut Self>, cx: &mut task::Context<'_>) -> task::Poll<Self::Output> {
        ...
        match this.state {
            State::Submitting { sq_id, mut cmd } => {
                ...
                if let Some((cq_id, cmd_id)) = Hw::try_submit(&executor.global_ctxt, sq_id, |cmd_id| {
...
unsafe fn vt_clone<Hw: Hardware>(idx: *const ()) -> task::RawWaker { task::RawWaker::new(idx, Hw::vtable()) }
unsafe fn vt_drop(_idx: *const ()) {}
unsafe fn vt_wake<Hw: Hardware>(idx: *const ()) {
    Hw::current().ready_futures.borrow_mut().push_back(idx as FutIdx);
}
...
impl Event {
    pub fn flags(&self) -> event::EventFlags { self.flags }
}
...
pub fn init_raw<Hw: Hardware>(global_ctxt: Hw::GlobalCtxt, vector: Hw::Iv, intx: bool, irq_handle: File) -> LocalExecutor<Hw> {
