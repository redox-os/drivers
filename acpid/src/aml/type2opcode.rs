...
use crate::monotonic;
use crate::acpi::{PossibleAmlTables, SdtSignature};
    ...
    loop {
        match ctx.acquire_mutex(ctx.acpi_context(), obj.val.clone()) {
            ...
            Ok(b) => if b {
                ...
                // TODO: How long should be sleep be?
    ...
    loop {
        match ctx.wait_for_event(ctx.acpi_context(), obj.val.clone()) {
            ...
            Ok(b) => if b {
                ...
                let (seconds, nanoseconds) = crate::monotonic();
                let current_time_ns = nanoseconds + (seconds * 1_000_000_000);
...
fn parse_def_load_table(data: &[u8],
    ...
    let sdt_signature = SdtSignature {
        signature,
        oem_id,
        oem_table_id,
    };

    let parse_len = 2 + signature.len() + oem_id.len() + oem_table_id.len() + root_path.len + parameter_path.len + parameter_data.len;
