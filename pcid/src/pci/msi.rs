...
impl MsiCapability {
    ...
    fn message_control_raw(&self) -> u32 {
        match self {
            ...
            Self::_32BitAddressWithPvm(s) => s.message_control(),
    ...
    pub fn set_message_control(&mut self, value: u16) {
        ...
        match self {
            ...
            Self::_32BitAddressWithPvm(ref mut s) => s.message_control = new_message_control,
    ...
    pub fn message_address(&self) -> u32 {
        match self {
            &Self::_32BitAddress(ref s) => s.message_address(),
            &Self::_32BitAddressWithPvm(ref s) => s.message_address(),
            &Self::_64BitAddress(ref s) => s.message_address(),
            &Self::_64BitAddressWithPvm(ref s) => s.message_address(),
        }
    }
    pub fn message_upper_address(&self) -> Option<u32> {
        match self {
            &Self::_64BitAddress(ref s) => s.message_upper_address(),
            &Self::_64BitAddressWithPvm(ref s) => s.message_upper_address(),
            &Self::_32BitAddress(_) | &Self::_32BitAddressWithPvm(_) => None,
        }
    }
    pub fn message_data(&self) -> u16 {
        match self {
            &Self::_32BitAddress(ref s) => s.message_data(),
            &Self::_64BitAddress(ref s) => s.message_data(),
            &Self::_32BitAddressWithPvm(ref s) => s.message_data(),
            &Self::_64BitAddressWithPvm(ref s) => s.message_data(),
        }
    }
    pub fn mask_bits(&self) -> Option<u32> {
        match self {
            &Self::_32BitAddressWithPvm(ref s) => s.mask_bits(),
            &Self::_64BitAddressWithPvm(ref s) => s.mask_bits(),

            &Self::_32BitAddress(_) | &Self::_64BitAddress(_) => None,
        }
    }
    pub fn pending_bits(&self) -> Option<u32> {
        match self {
            &Self::_32BitAddressWithPvm(ref s) => s.pending_bits(),
            &Self::_64BitAddressWithPvm(ref s) => s.pending_bits(),
            &Self::_32BitAddress(_) | &Self::_64BitAddress(_) => None,
        }
    }
    ...
    pub unsafe fn write_message_data<W: ConfigWriter>(&self, writer: &W, offset: u8) {
        // FIXME: Ensure that the bytes outside the capability struct when writing this are left
        // untouched.
