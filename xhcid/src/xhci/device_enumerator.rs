        ...
        loop {
            ...
            let (len, flags) = {
                ...
                if port_array_index as usize >= len{
                    warn!("Received out of bounds Device Enumeration request for port {}", request.port_number);
                    continue;
                }
            ...
            if flags.contains(PortFlags::PORT_CCS) {
                ...
                info!("Received Device Connect Port Status Change Event with port flags {:?}", flags);
                ...
                if !flags.contains(PortFlags::PORT_PED) {
                    ...
                    let disabled_state = flags.contains(PortFlags::PORT_PP) && flags.contains(PortFlags::PORT_CCS) &&
                        !flags.contains(PortFlags::PORT_PED) && !flags.contains(PortFlags::PORT_PR);

                    if !disabled_state{
                        panic!("Port {} isn't in the disabled state! Current flags: {:?}", request.port_number, flags);
                    } else {
                        debug!("Port {} has entered the disabled state.", request.port_number);
                    }
                    ...
                    let enabled_state = flags.contains(PortFlags::PORT_PP) && flags.contains(PortFlags::PORT_CCS) &&
                        flags.contains(PortFlags::PORT_PED) && !flags.contains(PortFlags::PORT_PR);

                    if !enabled_state{
                        warn!("Port {} isn't in the enabled state! Current flags: {:?}", request.port_number, flags);
                    } else {
                        debug!("Port {} is in the enabled state. Proceeding with enumeration", request.port_number);
                    }
                }
                ...
                let result = futures::executor::block_on(self.hci.detach_device(port_array_index as usize));
                match result {
                    Ok(_) => {
